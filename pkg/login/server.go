...
func receiveCodeViaLocalServer(ctx context.Context, c *ServerConfig) (string, error) {
	...
	eg.Go(func() error {
		defer close(respCh)
		c.Logf("starting a server at %s", l.Addr())
		defer c.Logf("stopped the server")
	...
	eg.Go(func() error {
		...
		c.Logf("shutting down the server")
		ctx, cancel := context.WithTimeout(ctx, 500*time.Millisecond)
		defer cancel()
		if err := server.Shutdown(ctx); err != nil {
			c.Logf("force-closing the server: shutdown failed: %s", err)
			_ = server.Close()
			return nil
		}
		return nil
...
func (h *localServerHandler) handleIndex(w http.ResponseWriter, r *http.Request) {
	...
	h.config.Logf("sending redirect to %s", authCodeURL)
